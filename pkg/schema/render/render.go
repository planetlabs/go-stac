...
// UnmarshalYamlSchema takes a []byte representation of the json-schema document and
// unmarshals it into a JSONSchemaType.
func UnmarshalYamlSchema(yamlSchemaDoc []byte) (*JSONSchema, error) {
	// Required is initialized with an empty array
	// otherwise it will hold an uninitialized slice when there are no required fields
	// this renders to null, which will break the json-schema parser
	// Type: "object" is set as a convenience, so that writers of properties schemas don't need
	// to add a boilerplate type attribute.
	jt := &JSONSchema{Type: "object", Required: make([]string, 0)}
	err := yaml.Unmarshal(yamlSchemaDoc, jt)
	return jt, err
}

// ExecuteCoreTemplate takes a JSONSchema object and uses it to render the core schema document template
func ExecuteCoreTemplate(s *JSONSchema, w io.Writer) error {
	name := s.Title
	required, err := json.Marshal(s.Required)
	if err != nil {
		return err
	}
	properties, err := json.Marshal(s.Properties)
	if err != nil {
		return err
	}

	return coreTemplate.Execute(w, struct {
		Name       string
		Required   string
		Properties string
	}{
...
func init() {
	...
	coreTemplate, err = template.New("core").Parse(coreTemplateString)
	if err != nil {
		panic(err)
	}
}

var coreTemplateString = `{
